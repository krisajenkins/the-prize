...
Elm.Types.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var movesFrom = function (_p0) {
      var _p1 = _p0;
      var _p3 = _p1._1;
      var _p2 = _p1._0;
      return $Set.fromList(_U.list([{ctor: "_Tuple2",_0: _p2 - 1,_1: _p3}
   ...
   var estimatedDistance = F2(function (_p5,_p4) {    var _p6 = _p5;var _p7 = _p4;return $Basics.abs(_p6._0 - _p7._0) + $Basics.abs(_p6._1 - _p7._1);});
   var objectAt = $Basics.flip($Dict.get);
   var canStandOn = function (cell) {
      var _p8 = cell;
      if (_p8.ctor === "Nothing") {
            return true;
         } else {
   ...
   var validMovesFrom = F2(function (world,position) {
      return A2($Set.filter,function (_p9) {    return canStandOn(A2(objectAt,world,_p9));},movesFrom(position));
   });
   var Model = F5(function (a,b,c,d,e) {    return {world: a,player: b,dialogue: c,destination: d,timeSinceLastMove: e};});
   var Player = function (a) {    return {position: a};};
   var Tick = function (a) {    return {ctor: "Tick",_0: a};};
   var MoveTo = function (a) {    return {ctor: "MoveTo",_0: a};};
   var Character = {ctor: "Character"};
   var Path = {ctor: "Path"};
   ...
   return _elm.Types.values = {_op: _op
                              ...
                              ,movesFrom: movesFrom
                              ,validMovesFrom: validMovesFrom};
};
Elm.Astar = Elm.Astar || {};
Elm.Astar.make = function (_elm) {
   "use strict";
   _elm.Astar = _elm.Astar || {};
   if (_elm.Astar.values) return _elm.Astar.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Array = Elm.Array.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var bestCost = F2(function (newDistance,oldDistance) {
      var _p0 = oldDistance;
      if (_p0.ctor === "Nothing") {
            return newDistance;
         } else {
            return A2($Basics.min,_p0._0,newDistance);
         }
   });
   var reconstructPath = F2(function (cameFrom,goal) {
      var _p1 = A2($Dict.get,goal,cameFrom);
      if (_p1.ctor === "Nothing") {
            return $Array.empty;
         } else {
            return A2($Array.push,goal,A2(reconstructPath,cameFrom,_p1._0));
         }
   });
   var updateCost = F3(function (next,neighbour,model) {
      var newCameFrom = A3($Dict.insert,neighbour,next,model.cameFrom);
      var distanceTo = $Array.length(A2(reconstructPath,newCameFrom,neighbour));
      var newModel = _U.update(model,{costs: A3($Dict.insert,neighbour,distanceTo,model.costs),cameFrom: newCameFrom});
      var _p2 = A2($Dict.get,neighbour,model.costs);
      if (_p2.ctor === "Nothing") {
            return newModel;
         } else {
            return _U.cmp(distanceTo,_p2._0) < 0 ? newModel : model;
         }
   });
   var cheapestOpen = F2(function (costFn,model) {
      return A2($Maybe.map,
      $Basics.fst,
      $List.head(A2($List.sortBy,
      $Basics.snd,
      A2($List.filterMap,
      function (position) {
         var _p3 = A2($Dict.get,position,model.costs);
         if (_p3.ctor === "Nothing") {
               return $Maybe.Nothing;
            } else {
               return $Maybe.Just({ctor: "_Tuple2",_0: position,_1: _p3._0 + costFn(position)});
            }
      },
      $Set.toList(model.openSet)))));
   });
   var astar = F4(function (costFn,movesFrom,goal,model) {
      astar: while (true) {
         var _p4 = A2(cheapestOpen,costFn(goal),model);
         if (_p4.ctor === "Nothing") {
               return $Maybe.Nothing;
            } else {
               var _p5 = _p4._0;
               if (_U.eq(_p5,goal)) return $Maybe.Just(A2(reconstructPath,model.cameFrom,goal)); else {
                     var neighbours = movesFrom(_p5);
                     var modelPopped = _U.update(model,{openSet: A2($Set.remove,_p5,model.openSet),evaluated: A2($Set.insert,_p5,model.evaluated)});
                     var newNeighbours = A2($Set.diff,neighbours,modelPopped.evaluated);
                     var modelWithNeighbours = _U.update(modelPopped,{openSet: A2($Set.union,modelPopped.openSet,newNeighbours)});
                     var modelWithCosts = A3($Set.foldl,updateCost(_p5),modelWithNeighbours,newNeighbours);
                     var _v5 = costFn,_v6 = movesFrom,_v7 = goal,_v8 = modelWithCosts;
                     costFn = _v5;
                     movesFrom = _v6;
                     goal = _v7;
                     model = _v8;
   ...
   var initialModel = function (start) {
      return {evaluated: $Set.empty,openSet: $Set.singleton(start),costs: A2($Dict.singleton,start,0),cameFrom: $Dict.empty};
   };
   var findPath = F4(function (costFn,movesFrom,start,goal) {    return A4(astar,costFn,movesFrom,goal,initialModel(start));});
   var Model = F4(function (a,b,c,d) {    return {evaluated: a,openSet: b,costs: c,cameFrom: d};});
   return _elm.Astar.values = {_op: _op,initialModel: initialModel,findPath: findPath};
};
Elm.State = Elm.State || {};
Elm.State.make = function (_elm) {
   ...
   var _op = {};
   var effects = F2(function (action,_p0) {    var _p1 = _p0;return _U.eq(_p1._1.destination,$Maybe.Nothing) ? $Effects.none : $Effects.tick($Types.Tick);});
   var initialEffects = $Effects.none;
   var wall = $List.map(function (coords) {    return {ctor: "_Tuple2",_0: coords,_1: $Types.Block};});
   var verticalWall = F2(function (x,ys) {    return wall(A2($List.map,function (y) {    return {ctor: "_Tuple2",_0: x,_1: y};},ys));});
   var horizontalWall = F2(function (xs,y) {    return wall(A2($List.map,function (x) {    return {ctor: "_Tuple2",_0: x,_1: y};},xs));});
   var initialWorld = function () {
      var _p2 = {ctor: "_Tuple2",_0: 15,_1: 7};
      var maxX = _p2._0;
      var maxY = _p2._1;
      var _p3 = {ctor: "_Tuple2",_0: 0,_1: 0};
      var minX = _p3._0;
      var minY = _p3._1;
      var xRange = _U.range(minX,maxX);
      var yRange = _U.range(minY,maxY);
      return $Dict.fromList($List.concat(_U.list([A2($List.concatMap,
                                                 function (x) {
                                                    return A2($List.map,
                                                    function (y) {
                                                       return {ctor: "_Tuple2",_0: {ctor: "_Tuple2",_0: x,_1: y},_1: $Types.Path};
                                                    },
                                                    yRange);
                                                 },
                                                 xRange)
                                                 ,A2(verticalWall,minX,yRange)
                                                 ,A2(verticalWall,maxX,yRange)
                                                 ,A2(horizontalWall,xRange,minY)
                                                 ,A2(horizontalWall,xRange,maxY)
                                                 ,A2(horizontalWall,_U.range(4,10),4)
                                                 ,A2(verticalWall,4,_U.range(4,6))
                                                 ,A2(verticalWall,8,_U.range(2,6))
                                                 ,A2(horizontalWall,_U.range(12,13),2)])));
   }();
   var initialModel = {world: initialWorld
                      ,dialogue: $Maybe.Just("Well, I guess I\'d better look around...")
                      ,player: {position: {ctor: "_Tuple2",_0: 3,_1: 3}}
                      ,destination: $Maybe.Nothing
                      ,timeSinceLastMove: $Maybe.Nothing};
   var stepTime = 100 * $Time.millisecond;
   var updateWithDialogue = F2(function (action,model) {
      var _p4 = action;
      if (_p4.ctor === "MoveTo") {
            var _p5 = _p4._0;
            return $Types.canStandOn(A2($Types.objectAt,model.world,_p5)) ? {ctor: "_Tuple2"
                                                                            ,_0: _U.update(model,{destination: $Maybe.Just(_p5)})
                                                                            ,_1: $Maybe.Just("Chaaaaarrrrrrge!")} : {ctor: "_Tuple2"
                                                                                                                    ,_0: model
                                                                                                                    ,_1: $Maybe.Just("Sorry, I can\'t walk through walls.")};
         } else {
            var _p10 = _p4._0;
            var _p6 = model.destination;
            if (_p6.ctor === "Nothing") {
                  return {ctor: "_Tuple2",_0: _U.update(model,{timeSinceLastMove: $Maybe.Nothing}),_1: $Maybe.Nothing};
               } else {
                  var previousTime = A2($Maybe.withDefault,0,model.timeSinceLastMove);
                  if (_U.cmp(previousTime + stepTime,_p10) > 0) return {ctor: "_Tuple2",_0: model,_1: $Maybe.Nothing}; else {
                        var _p7 = A4($Astar.findPath,$Types.estimatedDistance,$Types.validMovesFrom(model.world),model.player.position,_p6._0);
                        if (_p7.ctor === "Nothing") {
                              return {ctor: "_Tuple2"
                                     ,_0: _U.update(model,{destination: $Maybe.Nothing,timeSinceLastMove: $Maybe.Nothing})
                                     ,_1: $Maybe.Just("Hmm...I can\'t find a way there.")};
                           } else {
                              var _p8 = A2($Array.get,0,_p7._0);
                              if (_p8.ctor === "Nothing") {
                                    return {ctor: "_Tuple2"
                                           ,_0: _U.update(model,{destination: $Maybe.Nothing,timeSinceLastMove: $Maybe.Nothing})
                                           ,_1: function () {
                                              var _p9 = A2($Types.objectAt,model.world,model.player.position);
                                              if (_p9.ctor === "Nothing") {
                                                    return $Maybe.Nothing;
                                                 } else {
                                                    switch (_p9._0.ctor)
                                                    {case "Character": return $Maybe.Just("Hello.");
                                                       case "Path": return $Maybe.Just("It\'s nice here.");
                                                       default: return $Maybe.Just("Help, I\'m stuck in a wall!");}
                                                 }
                                           }()};
                                 } else {
                                    var player = model.player;
                                    return {ctor: "_Tuple2"
                                           ,_0: _U.update(model,{player: _U.update(player,{position: _p8._0}),timeSinceLastMove: $Maybe.Just(_p10)})
                                           ,_1: $Maybe.Nothing};
                                 }
                           }
                     }
   ...
   var update = F2(function (action,model) {
      var _p11 = A2(updateWithDialogue,action,model);
      var newModel = _p11._0;
      var newDialogue = _p11._1;
      return _U.eq(newDialogue,$Maybe.Nothing) ? newModel : _U.update(newModel,{dialogue: newDialogue});
   });
   return _elm.State.values = {_op: _op
                              ...
                              ,updateWithDialogue: updateWithDialogue
                              ,update: update
                              ,effects: effects};
};
Elm.Common = Elm.Common || {};
Elm.Common.View = Elm.Common.View || {};
Elm.Common.View.make = function (_elm) {
   "use strict";
   _elm.Common = _elm.Common || {};
   _elm.Common.View = _elm.Common.View || {};
   if (_elm.Common.View.values) return _elm.Common.View.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var pct = function (n) {    return A2($Basics._op["++"],$Basics.toString(n),"%");};
   var px = function (n) {    return A2($Basics._op["++"],$Basics.toString(n),"px");};
   return _elm.Common.View.values = {_op: _op,px: px,pct: pct};
};
Elm.View = Elm.View || {};
Elm.View.Svg = Elm.View.Svg || {};
Elm.View.Svg.make = function (_elm) {
   "use strict";
   _elm.View = _elm.View || {};
   _elm.View.Svg = _elm.View.Svg || {};
   if (_elm.View.Svg.values) return _elm.View.Svg.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Common$View = Elm.Common.View.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Dict = Elm.Dict.make(_elm),
   $Html = Elm.Html.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   var _op = {};
   var tileSize = 40;
   var tile = F3(function (address,position,cell) {
      var colours = function () {
         var _p0 = cell;
         switch (_p0.ctor)
         {case "Path": return _U.list([$Svg$Attributes.stroke("grey"),$Svg$Attributes.fill("#fdfdfd")]);
            case "Block": return _U.list([$Svg$Attributes.stroke("#0466da"),$Svg$Attributes.fill("#04c9da")]);
            default: return _U.list([$Svg$Attributes.stroke("#8504da"),$Svg$Attributes.fill("#e4049a")]);}
      }();
      return A2($Svg.rect,
      A2($Basics._op["++"],
      _U.list([$Svg$Attributes.x($Basics.toString($Basics.fst(position) * tileSize))
              ,$Svg$Attributes.y($Basics.toString($Basics.snd(position) * tileSize))
              ,$Svg$Attributes.width($Common$View.px(tileSize))
              ,$Svg$Attributes.height($Common$View.px(tileSize))
              ,$Svg$Events.onClick(A2($Signal.message,address,$Types.MoveTo(position)))]),
      colours),
      _U.list([]));
   });
   var root = F2(function (address,model) {
      return A2($Svg.svg,
      _U.list([$Svg$Attributes.width($Common$View.px(800)),$Svg$Attributes.height($Common$View.px(400))]),
      _U.list([A2($Svg.g,_U.list([]),A2($List.map,$Basics.uncurry(tile(address)),$Dict.toList(model.world)))
              ,A3(tile,address,model.player.position,$Types.Character)]));
   });
   return _elm.View.Svg.values = {_op: _op,root: root};
};
Elm.View = Elm.View || {};
Elm.View.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm),
   $Types = Elm.Types.make(_elm),
   $View$Svg = Elm.View.Svg.make(_elm);
   var _op = {};
   var root = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "margin",_1: "0 100px"}]))]),
      _U.list([A2($Html.div,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "perspective",_1: $Common$View.px(1000)}
                                                      ,{ctor: "_Tuple2",_0: "width",_1: $Common$View.px(800)}
                                                      ,{ctor: "_Tuple2",_0: "height",_1: $Common$View.px(300)}]))]),
              _U.list([A2($Html.div,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "transform",_1: "rotate3d(1,0,0,45deg)"}
                                                      ,{ctor: "_Tuple2",_0: "width",_1: $Common$View.pct(100)}
                                                      ,{ctor: "_Tuple2",_0: "height",_1: $Common$View.pct(100)}]))]),
              _U.list([A2($View$Svg.root,address,model)]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "font-size",_1: $Common$View.px(24)}]))]),
              _U.list([$Html.text(A2($Maybe.withDefault,"",model.dialogue))]))]));
   });
   return _elm.View.values = {_op: _op,root: root};
};
Elm.App = Elm.App || {};
Elm.App.make = function (_elm) {
   ...
   var _op = {};
   var app = $StartApp.start({init: {ctor: "_Tuple2",_0: $State.initialModel,_1: $State.initialEffects}
                             ,view: $View.root
                             ,update: F2(function (action,oldModel) {
                                var newModel = A2($State.update,action,oldModel);
                                var newEffects = A2($State.effects,action,{ctor: "_Tuple2",_0: oldModel,_1: newModel});
